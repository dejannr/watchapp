...
export default function ChatDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const chat = useQuery({
    ...
    queryFn: () => apiRequest<ChatDetail>(`/chats/${chatId}`, 'GET', undefined, true),
  });

  const messages = useQuery({
    queryKey: ['chat-messages', chatId],
    queryFn: () => apiRequest<ChatMessage[]>(`/chats/${chatId}/messages`, 'GET', undefined, true),
  ...
  const title = useMemo(() => chat.data?.listing?.title ?? 'Chat', [chat.data?.listing?.title]);
  const counterpartyLine = useMemo(() => {
    if (!chat.data || !currentUser) return 'Loading chat...';
    if (currentUser.id === chat.data.buyerId) {
      return `Seller: ${personName(chat.data.seller)}`;
    }
    if (currentUser.id === chat.data.sellerId) {
      return `Buyer: ${personName(chat.data.buyer)}`;
    }
    return 'Loading chat...';
  }, [chat.data, currentUser]);

  return (
    <div className="container space-y-4">
      <Link href="/chats" className="text-sm text-[var(--brand)]">Back to chats</Link>
      ...
      <div className="card space-y-3 p-4">
        <div className="max-h-[55vh] space-y-2 overflow-auto pr-1">
          {(messages.data ?? []).map((msg) => {
            const mine = currentUser?.id === msg.senderId;
            return (
              <div
                key={msg.id}
                className={`max-w-[85%] rounded border p-2 text-sm ${mine ? 'ml-auto bg-stone-100' : 'bg-white'}`}
              >
                <p className="text-xs text-[var(--muted)]">{mine ? 'You' : personName(msg.sender)}</p>
                <p>{msg.body}</p>
              </div>
            );
          })}
          {messages.isLoading && <p className="text-sm text-[var(--muted)]">Loading messages...</p>}
        </div>
