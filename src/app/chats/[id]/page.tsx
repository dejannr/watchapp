...
export default function RazgovorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const declineProposal = useMutation({
    mutationFn: async () => {
      await apiRequest(`/chats/${chatId}/sale-proposal/decline`, 'POST', {}, true);
    },
    onSuccess: async () => {
      setError('');
      setIsDealMenuOpen(false);
    ...
    onError: (e) => {
      setError(e instanceof ApiError ? e.message : 'Odbijanje predloga nije uspelo');
    },
  });
                        ...
                        {dealView.canAccept && (
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              ...
                              className="rounded-md bg-[var(--brand)] px-3 py-2 text-left text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
                              disabled={acceptProposal.isPending || declineProposal.isPending}
                            >
                              {acceptProposal.isPending ? 'Potvrda...' : 'Prihvati'}
                            </button>
                            <button
                              type="button"
                              onClick={() => declineProposal.mutate()}
                              className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-left text-xs font-semibold text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
                              disabled={acceptProposal.isPending || declineProposal.isPending}
                            >
                              {declineProposal.isPending ? 'Odbijanje...' : 'Odbij'}
                            </button>
                          </div>
                        )}
